...
{
    // Loading  Module
    torch::jit::script::Module module = torch::jit::load("../phone_detector.pt");
    std::cout << "1"; 
    ...
    cv::Mat imgr = cv::imread("../002_211112_148.jpg");
    cv::Mat img;

    cv::resize(imgr, img, cv::Size(320, 320));
    ...
    if (dets.size() > 0)
        {
            ...
            {
                float left = dets[0][i][0].item().toFloat() * img.cols / 320;
                float top = dets[0][i][1].item().toFloat() * img.rows / 320;
                float right = dets[0][i][2].item().toFloat() * img.cols / 320;
                float bottom = dets[0][i][3].item().toFloat() * img.rows / 320;
